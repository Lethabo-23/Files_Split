...
def split_excel(input_directory, output_directory, num_files):
    ...
    # Check if there is only one Excel file in the directory
    if len(excel_files) != 1:
        print("There should be one and only one Excel file in the input directory.")
        return
    ...
    for i in range(num_files):
        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            df_subset.to_excel(writer, index=False, sheet_name='Items Prices import')
...
if __name__ == "__main__":
    ...
    print(f"Excel file in the root folder split into {num_files} files in '{output_directory}'.")
